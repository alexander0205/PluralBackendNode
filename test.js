let data = [ { id: 9,
  ...
  { id: 24,
    ...
    from = '2018-04-01T00:00:00.000Z';  to ='2018-05-01T00:00:00.000Z'
 budget = { eating_out: 1000, groceries: 15000 }


function findCategoriesOverBudgetInPeriod (transactions, budget, from, to) {
        if(typeof transactions !== "object" || typeof budget !== "object")
          return {};


        const keysBudget = Object.keys(budget);
        let oDataGroupCat = {};
        from = new Date(from);
        to = new Date(to);

        //First filterProcess: is a funtion of validation, making more fast the process
        let transactionsAftherFilter = transactions.filter(x=>filterProcess(x,keysBudget, from, to));

        //Secound make all category in a group
         transactionsAftherFilter.forEach(x=>
         { 
            let sum = x.amount + (oDataGroupCat[x.category] == undefined?0:oDataGroupCat[x.category]);
            oDataGroupCat[x.category] =  sum;
         });

         //Third find if the value es correct
         Object.keys(oDataGroupCat).forEach(function(key) {
              let sumTotal =  oDataGroupCat[key] + budget[key];
              if(sumTotal <0)
                 oDataGroupCat[key] = Math.abs(sumTotal);
              else
                delete  oDataGroupCat[key];
          });

          return oDataGroupCat;
}

function filterProcess(x, keysBudget, startDate, endDate) {
    
    //First vCategory: Validate a single Category {Optional code:findIndex, but still this No support Internet Explorer}
    let vCategory = keysBudget.indexOf(x.category) >=0 ;

    //Secound vDates: Validate date.
    let vDates = (timeTransaction, dateFrom, dateTo) => {
          let timeTrasactionFormat = new Date(x.time);
          return timeTrasactionFormat >= dateFrom && timeTrasactionFormat < dateTo;
    }

    return vCategory &&  vDates(x.time,startDate,endDate) 
}